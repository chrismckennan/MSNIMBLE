...
Calc.FDP <- function(p.values, Beta, q.points = seq(0.05, 0.25, by=0.05), pi0=NULL, use.boot=F, ind.subset=NULL) {
  Beta <- Beta[!is.na(p.values)]
  if (!is.null(ind.subset)) {
    if (is.logical(ind.subset[1])) {
      ind.subset <- ind.subset[!is.na(p.values)]
    } else {
      ind.subset <- ind.subset[ind.subset%in%which(!is.na(p.values))]
    }
  }
  p.values <- p.values[!is.na(p.values)]
  if (use.boot && is.null(pi0)) {pi0 <- qvalue::pi0est(p = p.values, pi0.method = "bootstrap")$pi0}
  if (is.null(pi0)) {
  ...
  if (!is.null(ind.subset)) {
    q.values <- q.values[ind.subset]
    Beta <- Beta[ind.subset]
  }
